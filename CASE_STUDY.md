...
-   **Core AI Flows:**
    ...
    2.  **Content and Image Generation (`generate-structured-blog-post` and `generate-blog-cover-image`):** These two flows run in parallel. One takes the raw transcription and transforms it into a publish-ready article, applying user preferences and style guides. The other summarizes the content to create a prompt for a text-to-image model (Imagen 4), generating a relevant cover image.
