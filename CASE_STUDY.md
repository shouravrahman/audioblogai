...
-   **Background Jobs:** **Inngest** is used to manage our primary background task: generating an article. When a user uploads audio, a server action calls Inngest, which then orchestrates the multi-step process of transcription, content generation, and image creation. This non-blocking approach ensures the UI remains responsive and resilient to long-running AI tasks.
...
-   **Core AI Flows:**
    ...
    2.  **Content and Image Generation (`generate-structured-blog-post` and `generate-image`):** This is a sophisticated, multi-stage process. First, the `generate-structured-blog-post` flow takes the raw transcription and transforms it into a publish-ready article, applying user preferences. Crucially, it also identifies key points in the text to insert visual aids and embeds descriptive placeholders like `[image: A vibrant chart showing user growth over time]`. In parallel, a separate call to the `generate-image` flow creates a main cover image. Once the text and cover image are ready, the Inngest function parses all the `[image: ...]` placeholders, calls the `generate-image` flow for each one, and finally stitches the complete article together with all images in place.
